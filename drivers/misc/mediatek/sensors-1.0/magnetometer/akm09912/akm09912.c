...
#define MAG_ERR(fmt, args...)     pr_err(MAGN_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#else
#define MAGN_TAG
...
static int akm09912_suspend(struct device *dev);
static int akm09912_resume(struct device *dev);
...
static const struct dev_pm_ops akm09912_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(akm09912_suspend, akm09912_resume)
};
#endif
...
static int akm09912_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	return 0;
}

static int akm09912_flush(void)
{
			      ...
			      const struct i2c_device_id *id)
{
	...
	ctl.enable = akm09912_m_enable;
	ctl.set_delay = akm09912_m_set_delay;
	ctl.open_report_data = akm09912_m_open_report_data;
	ctl.batch = akm09912_batch;
	ctl.flush = akm09912_flush;
	...
	mag_data.get_data = akm09912_m_get_data;
