				...
				const struct i2c_device_id *id)
{
	...
	if (err) {
		ACC_PR_ERR("bmi160_acc_device register failed.\n");
...
static int __init bmi160_acc_init(void)
{
	...
	if (!node) {
		ACC_PR_ERR("Device Tree: can not find accel node!\n");
		return -1;
	}
	if (get_accel_dts_func(node, hw) < 0) {
		ACC_PR_ERR("Device Tree: get accel dts info failed!\n");
		return -1;
	}
	GSE_FUN();
